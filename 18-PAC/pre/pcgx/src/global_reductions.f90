...
!-----------------------------------------------------------------------
   ...
   interface global_sum0
     module procedure global_sum_dbl0
   end interface 

   interface global_sum1
     module procedure global_sum_dbl1
   end interface 

   interface global_sum2
     module procedure global_sum_dbl2
   end interface 
...
!-----------------------------------------------------------------------
   ...
   !print *, "kek, you should not call this!!!!"

 end function global_sum_dbl

  function global_sum_dbl0(X, dist, field_loc, MASK)
...
! !OUTPUT PARAMETERS:
   ...
   real (r8) :: &
      global_sum_dbl0       ! resulting global sum
...
!    endif
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_dbl0, 1, &
                            mpi_dbl, MPI_SUM, dist%communicator, ierr)!上面的大循环1%，而这里0.2%
      else
         global_sum_dbl0 = c0
      endif
   else
      global_sum_dbl0 = local_sum
   endif
   !call timer_stop(timer_mpi)


   else   ! lmic_proc
      global_sum_dbl0 = global_sum_dbl_tiny(X, dist, field_loc, MASK)
   endif  ! lmic_proc

 end function global_sum_dbl0

  function global_sum_dbl1(X, dist, field_loc, MASK)
...
! !OUTPUT PARAMETERS:
   ...
   real (r8) :: &
      global_sum_dbl1       ! resulting global sum
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_dbl1, 1, &
                            mpi_dbl, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_dbl1 = c0
      endif
   else
      global_sum_dbl1 = local_sum
   endif
   !call timer_stop(timer_mpi)


   else   ! lmic_proc
      global_sum_dbl1 = global_sum_dbl_tiny(X, dist, field_loc, MASK)
   endif  ! lmic_proc

 end function global_sum_dbl1

 function global_sum_dbl2(X, dist, field_loc, MASK)
...
! !OUTPUT PARAMETERS:
   ...
   real (r8) :: &
      global_sum_dbl2       ! resulting global sum
...
!-----------------------------------------------------------------------
   ...
   if (dist%nprocs > 1) then
      if (my_task < dist%nprocs) then
         call MPI_ALLREDUCE(local_sum, global_sum_dbl2, 1, &
                            mpi_dbl, MPI_SUM, dist%communicator, ierr)
      else
         global_sum_dbl2 = c0
      endif
   else
      global_sum_dbl2 = local_sum
   endif
   !call timer_stop(timer_mpi)


   else   ! lmic_proc
      global_sum_dbl2 = global_sum_dbl_tiny(X, dist, field_loc, MASK)
   endif  ! lmic_proc

 end function global_sum_dbl2
