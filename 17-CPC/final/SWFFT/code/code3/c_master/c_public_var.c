...
double* scratch_real_, *scratch_imag_;
//comm
...
double* c_u1_real_bf1_, *c_u1_imag_bf1_;
double* c_u2_real_bf1_, *c_u2_imag_bf1_;
double* c_u1_real_bf2_, *c_u1_imag_bf2_;
double* c_u2_real_bf2_, *c_u2_imag_bf2_;
double* c_u_real_, *c_u_imag_;
double* c_u_real_left_, *c_u_imag_left_;
double* c_u_real_right_, *c_u_imag_right_;
double* c_twiddle_;
//support
int chk_cnt_;
int* chk_xs_;
...
// (1 + 2047) -> (1 + 1023) + (1 + 1023)
void u_split(double* u, int n, double* out_left, double* out_right)
{
    int i;
    out_left[0] = 0;
    out_right[0] = 0;
    ...
    while((n >> 1) > 1)
    {
        int offset = (n >> 1);
        int len = (n >> 1);
        double* u_ptr = u + offset;
        double* out_left_ptr = out_left + (offset >> 1);
        double* out_right_ptr = out_right + (offset >> 1);

        for(i = 0; i < (len >> 1); ++i)
        {
            out_left_ptr[i] = u_ptr[2 * i];
            out_right_ptr[i] = u_ptr[2 * i + 1];
        }
        n = (n >> 1);
    }
                   ...
                   int* _xend,int *_yend,int *_zend, struct dcomplex* _sums)
{
  ...
  f_twiddle_ = _twiddle;
    ...
    //allocate
    scratch_imag_ = (double*)malloc(sizeof(my_fc) * fftblockpad_default_ * maxdim_ * 2);
    scratch_real_ = (double*)malloc(sizeof(my_fc) * fftblockpad_default_ * maxdim_ * 2);
    ...
    c_u1_real_bf1_ = (double*)f_u0_;
    c_u1_imag_bf1_ = (double*)f_u0_;
    c_u2_real_bf1_ = (double*)f_u2_;
    c_u2_imag_bf1_ = (double*)f_u2_;
    c_u_real_left_ = (double*)malloc(sizeof(double) * nx_);
    c_u_imag_left_ = (double*)malloc(sizeof(double) * nx_);
    c_u_real_right_ = (double*)malloc(sizeof(double) * nx_);
    c_u_imag_right_ = (double*)malloc(sizeof(double) * nx_);
    c_u_real_ = (double*)malloc(sizeof(double) * nx_);
    c_u_imag_ = (double*)malloc(sizeof(double) * nx_);

    c_u0_imag_ = c_u0_imag_ + ntdivnp_;
    c_u1_imag_bf1_ = c_u1_imag_bf1_ + ntdivnp_;
    c_u2_imag_bf1_ = c_u2_imag_bf1_ + ntdivnp_;

    c_u1_real_bf2_ = (double*)malloc(sizeof(double) * ntdivnp_);
    c_u1_imag_bf2_ = (double*)malloc(sizeof(double) * ntdivnp_);
    c_u2_real_bf2_ = (double*)malloc(sizeof(double) * ntdivnp_);
    c_u2_imag_bf2_ = (double*)malloc(sizeof(double) * ntdivnp_);

    assert(c_u_real_left_);
    assert(c_u_imag_left_);
    assert(c_u_real_right_);
    assert(c_u_imag_right_);
    assert(c_u_real_);
    assert(c_u_imag_);
    assert(c_u1_real_bf2_);
    assert(c_u1_imag_bf2_);
    assert(c_u2_real_bf2_);
    assert(c_u2_imag_bf2_);
    ...
    u_split(c_u_real_, 2048, c_u_real_left_, c_u_real_right_);
    u_split(c_u_imag_, 2048, c_u_imag_left_, c_u_imag_right_);
