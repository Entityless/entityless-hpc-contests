subroutine weno
    ...
    integer :: iper(3) !因为不知道save的实现方式，为了确保稳定的调用，将Iperiodic复制到这里
    integer :: ksshow, kssave
! allocate 
...
!----------------initial---------------------------------------------------------
    ...
    ksshow = Kstep_show
    kssave = Kstep_save
    call c_param_init(my_id, nx, ny, nz, npx, npy, npz, dt, end_time, hx, u, u1, iper, tt, istep, ksshow, kssave, LAP)
...
!
!            if(end_time .le. 0.d0) exit      ! end_time .le. 0  means that stop computation just after saving files 
!      !-------------------------------------------------------------------------
!        endif
!      !-----------------------------------------
!        if( tt .lt. end_time  .or. end_time .le. 0.d0) cycle
!        exit
!    end do
!--------------------------------------------------------------------------------------
...
subroutine check_u(u)
    Use OCFD_flow_parameters
    implicit none

    real(kind=OCFD_REAL_KIND)::u(1-LAP:nx+LAP,1-LAP:ny+LAP,1-LAP:nz+LAP)

    if(my_id == 0) print *, "check u = ", u(1, 1, 1), u(40, 40, 40)

end subroutine check_u

subroutine check_u1(u1)
    Use OCFD_flow_parameters
    implicit none

    real(kind=OCFD_REAL_KIND)::u1(nx,ny,nz)

    if(my_id == 0) print *, "check u1 = ", u1(1, 1, 1), u1(40, 40, 40)

end subroutine check_u1

subroutine wtime_util(tm)
    Use OCFD_flow_parameters
    implicit none
...
end subroutine wtime_util

subroutine ocfd_save_util(u)
    Use OCFD_flow_parameters
    implicit none
