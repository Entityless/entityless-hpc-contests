...
!     * character :: action = 'create', 'define', 'write' or 'read'. One character is also
...
!*DeckYinxq: writenc_0d_text
  ...
  if (.not.present(RecNum))then
    ...
    status=NF_PUT_VARA_text(ncID, VarID, [1, RecNum], [len_trim(var), 1], Var)
  endif
  IF(status.NE.NF_NOERR) CALL check_err(status)
...
!*DeckYinxq: readnc_0d_text
  ...
  if (.not.present(RecNum))then
    ...
    status = NF_GET_VARA_text(ncID, varID, [1, RecNum], [len(var), 1], Var)
  endif
