...
void allin_allocate()
{
     cpe_u_real_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx / 2));
     cpe_u_imag_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx / 2));

//     cpe_u1_real_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx / 2));
//     cpe_u1_imag_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx / 2));

     cpe_u1_real_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx));
     cpe_u1_imag_ = cpe_u1_real_ + param.nx / 2;//then you can use u1_real as buffer array

     cpe_u2_imag_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx / 2));
     cpe_u2_real_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx / 2));

     cpe_twiddle_ = (double*)nw_ldm_malloc(sizeof(double) * (param.nx / 2));
}
...
void allin_initial_transfer()
{
    ...
    {
        athread_get(
            PE_MODE, (void*)param.u_real_left, (void*)cpe_u_real_,
            sizeof(double) * param.nx / 2, (void*)(&get_reply), 0, 0, 0
        );
        athread_get(
            PE_MODE, (void*)param.u_imag_left, (void*)cpe_u_imag_,
            sizeof(double) * param.nx / 2, (void*)(&get_reply), 0, 0, 0
        );
    }
    else
    {
        athread_get(
            PE_MODE, (void*)param.u_real_right, (void*)cpe_u_real_,
            sizeof(double) * param.nx / 2, (void*)(&get_reply), 0, 0, 0
        );
        athread_get(
            PE_MODE, (void*)param.u_imag_right, (void*)cpe_u_imag_,
            sizeof(double) * param.nx / 2, (void*)(&get_reply), 0, 0, 0
        );
    }
    while(get_reply != 2);
...
void sg_evolve(int d1)
{
    ...
    for(i = 0; i < d1; i++)
    {
        cpe_u2_real_[i] = cpe_u2_real_[i] * cpe_twiddle_[i];
        cpe_u2_imag_[i] = cpe_u2_imag_[i] * cpe_twiddle_[i];
    }
}
...
void read_in_twiddle_half(int mpe_offset)
{
    get_reply = 0;
    athread_get(
...
void read_in_my_u_half(double *ux_real, double *ux_imag, double *mpe_ux_real, double *mpe_ux_imag, int mpe_offset)
{
    get_reply = 0;
    athread_get(
                PE_MODE, (void*)&mpe_ux_real[mpe_offset], (void*)&ux_real[0],
...
void write_out_my_u_half(double* ux_real, double* ux_imag, double* mpe_ux_real, double* mpe_ux_imag, int mpe_offset)
{
    put_reply = 0;
    athread_put(
                PE_MODE, (void*)&ux_real[0], (void*)&mpe_ux_real[mpe_offset],
            sizeof(double) * param.nx / 2, (void*)(&put_reply), 0, 0
            );
    athread_put(
                PE_MODE, (void*)&ux_imag[0], (void*)&mpe_ux_imag[mpe_offset],
            sizeof(double) * param.nx / 2, (void*)(&put_reply), 0, 0
...
void simd_fft_general_2step(double* real, double* imag, double* ureal, double* uimag,
                            double* dest_real, double* dest_imag,
                            int load_stride, int store_stride,
                            int u_offset, int u_len)
{
    doublev4 a1_real; simd_load(a1_real, real + 0 * load_stride + 0);
    doublev4 a2_real; simd_load(a2_real, real + 0 * load_stride + 4);
    doublev4 a3_real; simd_load(a3_real, real + 1 * load_stride + 0);
    doublev4 a4_real; simd_load(a4_real, real + 1 * load_stride + 4);
    doublev4 a5_real; simd_load(a5_real, real + 2 * load_stride + 0);
    doublev4 a6_real; simd_load(a6_real, real + 2 * load_stride + 4);
    doublev4 a7_real; simd_load(a7_real, real + 3 * load_stride + 0);
    doublev4 a8_real; simd_load(a8_real, real + 3 * load_stride + 4);

    doublev4 a1_imag; simd_load(a1_imag, imag + 0 * load_stride + 0);
    doublev4 a2_imag; simd_load(a2_imag, imag + 0 * load_stride + 4);
    doublev4 a3_imag; simd_load(a3_imag, imag + 1 * load_stride + 0);
    doublev4 a4_imag; simd_load(a4_imag, imag + 1 * load_stride + 4);
    doublev4 a5_imag; simd_load(a5_imag, imag + 2 * load_stride + 0);
    doublev4 a6_imag; simd_load(a6_imag, imag + 2 * load_stride + 4);
    doublev4 a7_imag; simd_load(a7_imag, imag + 3 * load_stride + 0);
    doublev4 a8_imag; simd_load(a8_imag, imag + 3 * load_stride + 4);

    double* ureal_ptr = ureal + u_offset;
    double* uimag_ptr = uimag + u_offset;
    int u_stride = u_len >> 1;
    ...
    simd_store(a1_real, dest_real + 0 * store_stride + 0);
    simd_store(a2_real, dest_real + 0 * store_stride + 4);
    simd_store(a5_real, dest_real + 1 * store_stride + 0);
    simd_store(a6_real, dest_real + 1 * store_stride + 4);
    simd_store(a3_real, dest_real + 2 * store_stride + 0);
    simd_store(a4_real, dest_real + 2 * store_stride + 4);
    simd_store(a7_real, dest_real + 3 * store_stride + 0);
    simd_store(a8_real, dest_real + 3 * store_stride + 4);

    simd_store(a1_imag, dest_imag + 0 * store_stride + 0);
    simd_store(a2_imag, dest_imag + 0 * store_stride + 4);
    simd_store(a5_imag, dest_imag + 1 * store_stride + 0);
    simd_store(a6_imag, dest_imag + 1 * store_stride + 4);
    simd_store(a3_imag, dest_imag + 2 * store_stride + 0);
    simd_store(a4_imag, dest_imag + 2 * store_stride + 4);
    simd_store(a7_imag, dest_imag + 3 * store_stride + 0);
    simd_store(a8_imag, dest_imag + 3 * store_stride + 4);

}

void cpe_fft_dual_batch8(int my_core, double* real, double* imag, int batch8_num,
                        double last_ureal, double last_uimag,
                        double* dest_real, double* dest_imag)
{
    int i;
    int my_rank = my_core % SIMD_COMM_SIZE;
    int partner = my_rank ^ 1;
    doublev4 u4_real = last_ureal;
    doublev4 u4_imag = last_uimag;
    if(my_rank < partner)
    {
        int left = my_rank;
        int right = partner;
        for(i = 0; i < batch8_num; ++i)
        {
            doublev4 left_real1; simd_load(left_real1, real + i * 8    );
            volatile doublev4 left_real2; simd_load(left_real2, real + i * 8 + 4);
            doublev4 left_imag1; simd_load(left_imag1, imag + i * 8    );
            volatile doublev4 left_imag2; simd_load(left_imag2, imag + i * 8 + 4);
            asm volatile ("nop":::"memory");
            __builtin_sw64_putR(left_real2, right);
            asm volatile ("nop":::"memory");
            __builtin_sw64_putR(left_imag2, right);
            asm volatile ("nop":::"memory");
            left_real2 = __builtin_sw64_getxR(left_real2);
            asm volatile ("nop":::"memory");
            left_imag2 = __builtin_sw64_getxR(left_imag2);
            fft_kernel(left_real1, left_imag1, left_real2, left_imag2, u4_real, u4_imag)
            ...
            asm volatile ("nop":::"memory");
            left_imag2 = __builtin_sw64_getxR(left_imag2);
            simd_store(left_real1, dest_real + i * 8    );
            simd_store(left_real2, dest_real + i * 8 + 4);
            simd_store(left_imag1, dest_imag + i * 8    );
            simd_store(left_imag2, dest_imag + i * 8 + 4);
        }
    }
    else
    {
        ...
        for(i = 0; i < batch8_num; ++i)
        {
            volatile doublev4 left_real1; simd_load(left_real1, real + i * 8    );
            doublev4 left_real2; simd_load(left_real2, real + i * 8 + 4);
            volatile doublev4 left_imag1; simd_load(left_imag1, imag + i * 8    );
            doublev4 left_imag2; simd_load(left_imag2, imag + i * 8 + 4);
            asm volatile ("nop":::"memory");
            __builtin_sw64_putR(left_real1, left);
            asm volatile ("nop":::"memory");
            __builtin_sw64_putR(left_imag1, left);
            asm volatile ("nop":::"memory");
            left_real1 = __builtin_sw64_getxR(left_real1);
            asm volatile ("nop":::"memory");
            left_imag1 = __builtin_sw64_getxR(left_imag1);
            fft_kernel(left_real1, left_imag1, left_real2, left_imag2, u4_real, u4_imag)
...
void simd_fft1024(double* real, double* imag, double* ureal, double* uimag, double* dest_real, double* dest_imag)
{
    ...
    for(i = 0; i < 8; ++i)
    {
        simd_fft_general_3step(dest_real + (bk * 8 + i) * 4, dest_imag + (bk * 8 + i) * 4, ureal, uimag,
                               real + i * 4 + bk * 256, imag + i * 4 + bk * 256,
                               128, 32,
                               16 + bk, 16);
    }
    ...
    for(i = 0; i < 32; ++i)
    {
        simd_fft_general_2step(real + i * 8, imag + i * 8, ureal, uimag,
                               dest_real + i * 8, dest_imag + i * 8,
                               256, 256,
                               2, 2);
    }
}

#define simd_xshuff(a, b)                   \
{                                           \
    doublev4 temp = simd_vshff(b, a, 0x88); \
    b = simd_vshff(b, a, 0xDD);             \
    a = temp;                               \
}

void cpe_butexchange(int my_core, int left, int right, double* data, int batch8_num, double* dest)
{
    int i;
    int offset = batch8_num * 4;
    if((my_core % SIMD_COMM_SIZE) == left)
    {
        for(i = 0; i < batch8_num; ++i)
        {
            volatile doublev4 send1, send2;
            simd_load(send1, data + i * 8    );
            simd_load(send2, data + i * 8 +4);
            simd_xshuff(send1, send2);

            asm volatile ("nop":::"memory");
            __builtin_sw64_putR(send2, right);
            asm volatile ("nop":::"memory");
            send2 = __builtin_sw64_getxR(send2);
            simd_store(send1, dest + i * 4         );
            simd_store(send2, dest + i * 4 + offset);
        }
    }
    else if((my_core % SIMD_COMM_SIZE) == right)
    {
        for(i = 0; i < batch8_num; ++i)
        {
            volatile doublev4 send1, send2;
            ...
            asm volatile ("nop":::"memory");
            __builtin_sw64_putR(send1, left);
            asm volatile ("nop":::"memory");
            send1 = __builtin_sw64_getxR(send1);
            simd_store(send1, dest + i * 4         );
            simd_store(send2, dest + i * 4 + offset);
        }
    }
...
void cpe_adj_butexchange(int my_core, double* data, int batch8_num, double* dest)
{
    int my_rank = my_core % SIMD_COMM_SIZE;
    int partner = my_rank ^ 1;
    if(my_rank < partner)
    {
        cpe_butexchange(my_core, my_rank, partner, data, batch8_num, dest);
    }
    else
    {
        cpe_butexchange(my_core, partner, my_rank, data, batch8_num, dest);
    }
}


void simd_fft2048(int my_core, double* real, double* imag,
                  double* ureal, double* uimag,
                  double* dest_real, double* dest_imag,
                  double last_ureal, double last_uimag)
{
    cpe_adj_butexchange(my_core, real, 128, dest_real);
    cpe_adj_butexchange(my_core, imag, 128, dest_imag);
    simd_fft1024(dest_real, dest_imag, ureal, uimag, real, imag);
    cpe_fft_dual_batch8(my_core, real, imag, 128,
                        last_ureal, last_uimag,
                        dest_real, dest_imag);
}
...
void cp_evv_fft()
{
    ...
    int my_rank = my_core % SIMD_COMM_SIZE;
    int my_couple = my_core / 2;
    int partner = my_rank ^ 1;//this is the id in cpes matrix row
    int couple_2048_off = ((my_rank - partner + 1) / 2) * 1024;//if my partner is larger than me, then my offset will be 0; else be 1024
    ...
    my_start = BLOCK_LOW(my_couple, 32, loop_length * 32 + remain_length);
    my_len = BLOCK_SIZE(my_couple, 32, loop_length * 32 + remain_length);
    ...
    for(ig = 0; ig < loop_length + 1; ig++)
    {
        ...
        read_in_my_u_half(cpe_u2_real_, cpe_u2_imag_, ptr_in_real, ptr_in_imag, jkid * in_stride + couple_2048_off);
        read_in_twiddle_half(jkid * in_stride + couple_2048_off);

        sg_evolve(fft_d1 / 2);
        write_out_my_u_half(cpe_u2_real_, cpe_u2_imag_, ptr_in_real, ptr_in_imag, jkid * out_stride + couple_2048_off);

        simd_fft2048(my_core, cpe_u2_real_, cpe_u2_imag_, cpe_u_real_, cpe_u_imag_, cpe_u1_real_, cpe_u1_imag_, param.last_ureal, param.last_uimag);
...
//        }
        ...
        write_out_my_u_half(cpe_u1_real_, cpe_u1_imag_, ptr_out_real, ptr_out_imag, jkid * out_stride + couple_2048_off);
    }
...
void tp_x_y()
{
    ...
    slave_tanspose_zyx2zxy(my_core, cpe_u1_real_, in_ptr, out_ptr, d1, d2, d3);
...
void tp_local()
{
    ...
    c_transpose2_local(my_core, cpe_u1_real_, in_ptr, out_ptr, d1, d2, d3);
...
//               arr_len_, part_len_, cpe_cbw_, cpe_cbw_vol_);
    ...
    while(1)
    {
        ...
        if (local_flag[KERNEL_ACTION] == CP_EVV_FFT_FLAG)
        {
            cp_evv_fft();
            tp_local();
        }
